...
- Khi `done`: lưu `chat_messages` (role=assistant, content=full text ghép từ các token) + insert `message_citations` — `session_id` lúc này là session thật đã tồn tại từ Phase 6, không còn vấn đề khóa ngoại.
  **Cập nhật (2026-08-23):** việc lưu này KHÔNG còn là việc mới của phase 7 — [Phase 6](phase-6-chat-sessions.md) (mục ③④⑥⑦, bước 6.4–6.6) đã làm đầy đủ cho nhánh non-streaming, gồm cả: commit tin nhắn người dùng trước khi gọi LLM (vì `now()` của Postgres cố định trong 1 transaction), "chạm" `updated_at` của session, lưu `message_citations`, và ghi `ai_usage_log_id` vào `chat_messages.metadata` để feedback 👍/👎 tải lại được. Phase 7 chỉ cần **tái sử dụng đúng lối lưu đó** ở thời điểm `done`, KHÔNG viết lại logic lưu song song — nếu viết lại, rất dễ sót 1 trong 4 điểm trên và tạo ra khác biệt âm thầm giữa 2 nhánh streaming/non-streaming.
- **Xoá endpoint tạm `POST /api/documents/{id}/ask`** ở phase này (đã quyết ở [Phase 6](phase-6-chat-sessions.md) mục ⑧: giữ nguyên trong Phase 6 vì frontend còn đang chạy trên nó, xoá khi frontend đã chuyển hẳn sang session API). Kiểm tra kỹ trước khi xoá: endpoint feedback `POST /api/documents/{id}/ask/{answer_id}/feedback` **phải giữ lại** — nó không phụ thuộc `/ask` và vẫn là đường gửi 👍/👎 duy nhất.
